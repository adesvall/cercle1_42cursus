...
/*   Updated: 2021/01/12 21:11:59 by adesvall         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned char	*file_header_bmp(int filesize, t_scn *scn)
{
	...
	if(!(bmpfileheader = (unsigned char*) malloc(14 * sizeof(unsigned char))))
		handle_error("Malloc failed", 20, scn);
...
void			write_data(int f, const unsigned char *data, t_couple res, const int filesize)
{
	int 			line;
...
void			save_bmp(const char *filename, const unsigned char *data, t_scn *scn)
{
	...
	free(bmpinfoheader);
	write_data(f, data, scn->res, filesize);
	close(f);
}
